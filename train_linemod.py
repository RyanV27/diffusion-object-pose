...
# --------------------------------------------------
# Changing the batch size to prevent CUDA out of memory error
config_run.train.batch_size = 16
# --------------------------------------------------
...
# Split 1: Epoch 6 (To do next)
# Split 2: Epoch 5 (Current)
# Split 3: Epoch 5 (To do next)
for epoch in tqdm(range(5, config_run.train.epochs)):
    if epoch > 0:
        print(f"Loading previous epoch {epoch - 1} checkpoint...")
        weights.load_prev_epoch_checkpoint(model=model, 
                                           pth_path=os.path.join(save_path, 'model_epoch{}.pth'.format(epoch - 1)))
    ...
    print(f"Saved aggregation network checkpoint at training epoch {epoch}")