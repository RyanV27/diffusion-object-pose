...
def crop_dataset(idx_obj, dataset, root_path, save_dir, crop_size, split=None):
    ...
    if dataset == "LINEMOD":
        ...
        # num_images = len(os.listdir(img_obj_dir)) - 1
    elif dataset == "occlusionLINEMOD":
        ...
        # num_images = 302 if split == "test" else 1543

    print(f"No of images in {img_obj_dir} = {num_images}")
    
    for idx_frame in tqdm(range(num_images)):
    # for idx_frame in tqdm(range(num_images - 1)):
        if(os.path.exists(os.path.join(save_obj_crop_dir, "{:06d}.png".format(idx_frame))) or (not os.path.exists(os.path.join(img_obj_dir, img_name.format(idx_frame))))):
            continue
