...
# I added this
def load_prev_epoch_checkpoint(model, pth_path):
    # load checkpoint for aggregation network in the previous training epoch
    checkpoint = torch.load(pth_path, map_location=lambda storage, loc: storage.cuda())
    state_dict = checkpoint['model']
    aggr_model_dict = model.aggregation_network.state_dict()
    aggr_model_dict.update(state_dict)
    model.aggregation_network.load_state_dict(aggr_model_dict, strict=True)
    print("Load previous epoch checkpoint done!")
